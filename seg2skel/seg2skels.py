...
if __name__ == '__main__':
    ...
    parser.add_argument('--segment-folder', default=program_dir+'assets/datasets/segmentation_results/', type=str)
    parser.add_argument('--assets-dir', default=program_dir+'seg2skel/', type=str)
    parser.add_argument('--min-size', default=500, type=int)
    args = parser.parse_args()

    photos = glob.glob(os.path.join(args.segment_folder, '*_image.png'))
    text = Text(args.assets_dir)
    skel = Skeleton()
    for fpath in photos:
        ...
        if ori_img.shape[0] < args.min_size and ori_img.shape[1] < args.min_size:
            continue
