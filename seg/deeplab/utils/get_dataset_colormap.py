...
def create_homeplus_label_colormap():
  colormap = np.zeros((_DATASET_MAX_ENTRIES[_PASCAL], 3), dtype=int)
  label_colors = [
    [0, 0, 0],
    [255, 0, 0],
  ...
  for i in range(_DATASET_MAX_ENTRIES[_HOMEPLUS]):
      if i < len(label_colors):
          colormap[i, :] = label_colors[i]
      else:
          colormap[i, :] = [i, i, i]
  return colormap
...
def create_label_colormap(dataset=_PASCAL):
  ...
  elif dataset == _HOMEPLUS:
    return create_homeplus_label_colormap()
  else:
